-------------------------------导入自定义配方----------------------------------
local recipes_status,recipes_data = pcall(require,"decoration_recipes")
...
local actions_status,actions_data = pcall(require,"decoration_actions")
...
if actionqueuer_status then
	...
	AddComponentPostInit("actionqueuer", function(self)
		local oldDropActiveItem = self.DropActiveItem
		self.DropActiveItem = function(self,pos,item)
			if self.inst:HasTag("item_decorator") and item and item:HasTag("isdecorationitem") then
				local active_item = self:GetActiveItem() or self:GetNewActiveItem(item.prefab)
				if not active_item then return false end
				if #TheSim:FindEntities(pos.x, 0, pos.z, 0.1, nil, {"DECOR", "FX", "INLIMBO", "NOCLICK", "player"}) == 0 then
					local act = BufferedAction(self.inst, nil, ACTIONS.DEPLOYDECORATION, active_item, pos)
					act.options.wholestack = false
					self:SendActionAndWait(act, false)
				end
				return true
			end
			return oldDropActiveItem and oldDropActiveItem(self,pos,item)
		end

		local oldDeployActiveItem = self.DeployActiveItem
		self.DeployActiveItem = function(self,pos,item)
			...
			return oldDeployActiveItem and oldDeployActiveItem(self,pos,item)
		end

		local oldDeployToSelection = self.DeployToSelection
		self.DeployToSelection = function(self, deploy_fn, spacing, item)
			if self.inst:HasTag("item_decorator") and item and item:HasTag("isdecorationitem") then
				spacing = 0.5
			end
			return oldDeployToSelection and oldDeployToSelection(self, deploy_fn, spacing, item)
		end
	end)
end